...
function BusinessViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface>(
    ...
    () =>
      getBusinessById(id, {
        relations: ['user', 'service', 'subscription'],
      }),
  );
              ...
              {hasAccess('service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/services/create?business_id=${data?.id}`}>
                      ...
                      <Tbody>
                        {data?.service?.map((record) => (
                          <Tr
                            ...
                            key={record.id}
                          >
                            <Td>{record.pickup_time as unknown as string}</Td>
                            <Td>{record.dropoff_time as unknown as string}</Td>
                            <Td>{record.status}</Td>
                            <Td>
              ...
              {hasAccess('subscription', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/subscriptions/create?business_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
